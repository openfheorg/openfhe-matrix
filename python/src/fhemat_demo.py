import numpy as np
import fhematrix as fm
from fhematrix import FHEMatrix as fmat
...
def demo():
    ...
    ctm_a = fmat.array(cc, keys.publicKey, a, total_slots)
    ctm_b = fmat.array(cc, keys.publicKey, b, total_slots)

    ctm_c = fmat.array(cc, keys.publicKey, c, total_slots, block_size)
    # ctm_d = fmat.array(cc, keys.publicKey, d, total_slots, block_size)

    print("Matrix multiplication:")
    ct_prod = fm.mms_mult(cc, keys, ctm_a, ctm_b)
    result = ct_prod.decrypt(cc, keys.secretKey)
    print(result)

    print("Matrix Vector multiplication: A@c")

    sum_col_keys = fm.gen_sum_col_keys(cc, keys.secretKey, block_size)

    ct_prod = fm.mv_mult(cc, keys, sum_col_keys, fm.MM_CRC, block_size, ctm_c, ctm_a)
    result = fm.decrypt(cc, keys.secretKey, ct_prod, block_size, 1)
    print(result)
